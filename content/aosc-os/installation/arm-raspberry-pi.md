...
With community support you can enjoy AOSC OS in your Raspberry Pi easily.  
There are many way to boot Linux on a Raspberry Pi, e.g. UEFI, U-Boot, etc. This tutorial is meant for plain boot method.

Following this tutorial, you will bootstrap the entire OS from existing AOSC OS installation.

# Supported boards

Technically all Raspberry Pis equipped with an ARM64 core are compatible with mainline AOSC OS, including:
...
- Raspberry Pi 3th gen boards (BCM2837A0/B0)
  ...
  - Compute Module 3
- Some BCM2837 based Raspberry Pi 2 boards (2B Rev1.2, slower)

Currently only Raspberry Pi 4B and 400 are supported by our community.
...
- A storage device to install OS. It can be a SD card, or a SSD/HDD with USB closure.
...
If your host machine is not arm64 based, you should set up binfmt to help you do the post-installation process.
...
This command will create partirons on `/dev/sda` with GPT map and a 200MiB boot partiton:
...
This command will create partitions on `/dev/sda` with MBR map and a 200MiB boot partiton:
...
Then grab the latest AOSC OS ARM64 tarball you want, and then extract:
...
Or if you want to run with LTS kernel, install `linux+kernel+rpi64+lts` instead.
